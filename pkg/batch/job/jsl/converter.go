...
import (
	...
	reader "sample/pkg/batch/step/reader"
	writer "sample/pkg/batch/step/writer"
	...
	for id, element := range jslFlow.Elements {
		...
		if err := yaml.Unmarshal(elementBytes, &jslStep); err == nil && (jslStep.Reader.Ref != "" || jslStep.Tasklet.Ref != "") {
			...
			if jslStep.Chunk != nil {
				...
				r, isReader := readerComp.(reader.Reader[any])
				if !isReader {
					return nil, exception.NewBatchErrorf(module, "リーダー '%s' が見つからないか、不正な型です (期待: Reader[any], 実際: %s)", jslStep.Reader.Ref, reflect.TypeOf(readerComp))
				}
				...
				p, isProcessor := processorComp.(processor.Processor[any, any])
				if !isProcessor {
					return nil, exception.NewBatchErrorf(module, "プロセッサ '%s' が見つからないか、不正な型です (期待: Processor[any, any], 実際: %s)", jslStep.Processor.Ref, reflect.TypeOf(processorComp))
				}
				...
				w, isWriter := writerComp.(writer.Writer[any])
				if !isWriter {
					return nil, exception.NewBatchErrorf(module, "ライター '%s' が見つからないか、不正な型です (期待: Writer[any], 実際: %s)", jslStep.Writer.Ref, reflect.TypeOf(writerComp))
				}
