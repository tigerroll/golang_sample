...
func (ec ExecutionContext) GetNested(key string) (interface{}, bool) {
	...
	// キーがネストされていない場合、またはトップレベルキーとして直接アクセスする場合
	if len(parts) == 1 {
		return ec.Get(key)
	}

	var currentMap interface{} = ec // interface{} として開始
	var ok bool = true
	...
	for i, part := range parts {
		...
		// currentMap が map[string]interface{} または ExecutionContext であることを確認
		...
		currentMap, ok = nextMap[part] // 次のレベルのマップまたは値を取得
		if !ok {
			return nil, false
		}

		// 最後のパートでない場合、次の値がマップであることを期待
		if i < len(parts)-1 {
...
func (ec ExecutionContext) PutNested(key string, value interface{}) {
	parts := strings.Split(key, ".")
	currentMap := ec // ExecutionContext は map[string]interface{} のエイリアス

	for i, part := range parts {
		if i == len(parts)-1 { // 最後のパート
			currentMap[part] = value // 直接マップに設定
		} else { // 中間パート
			nextMapVal, ok := currentMap[part] // 直接マップから取得
			var nextMap ExecutionContext
			if !ok {
				nextMap = NewExecutionContext() // Creates ExecutionContext
				currentMap[part] = nextMap      // 直接マップに設定
			} else {
				// 既存の値が ExecutionContext または map[string]interface{} であることを確認
				if m, isEC := nextMapVal.(ExecutionContext); isEC {
					nextMap = m
				} else if m, isMap := nextMapVal.(map[string]interface{}); isMap {
					nextMap = ExecutionContext(m) // map[string]interface{} を ExecutionContext に変換
				} else {
			...
			currentMap = nextMap // 次のイテレーションのために currentMap を更新
		}
	}
}
