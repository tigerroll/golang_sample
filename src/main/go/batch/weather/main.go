...
//go:embed resources/jobs
var embeddedJSL embed.FS // JSL YAML ファイルを埋め込む (resources/jobs ディレクトリ内の全yamlファイルを対象)
...
func main() {
	...
	if err := jsl.LoadJSLDefinitions(embeddedJSL, "jobs"); err != nil { // ★ 修正: 埋め込みJSLファイルとサブパスをロード
